...
MediaSampleAmigaOS::MediaSampleAmigaOS(RefPtr<Acinerella::AcinerellaPackage>& sample, const FloatSize& presentationSize, const String& trackId)
...
MediaSampleAmigaOS::~MediaSampleAmigaOS()
{
	D(dprintf("~MediaSample: pts %f\n", m_pts.toFloat()));
}

Ref<MediaSample> MediaSampleAmigaOS::createNonDisplayingCopy() const
{
	RefPtr<Acinerella::AcinerellaPackage> ptr(m_sample.get());
	return MediaSampleAmigaOS::create(ptr, m_presentationSize, m_trackId);
}

void MediaSampleAmigaOS::offsetTimestampsBy(const MediaTime& timestampOffset)
{
    if (!timestampOffset)
        return;
...
void MediaSampleAmigaOS::setTimestamps(const MediaTime& pts, const MediaTime& dts)
{
	m_pts = pts;
	m_dts = dts;
}

PlatformSample MediaSampleAmigaOS::platformSample() const
{
	PlatformSample sample = { PlatformSample::AmigaOSSampleType, { .mosSample = m_sample.get() } };
	return sample;
}

std::optional<MediaSample::ByteRange> MediaSampleAmigaOS::byteRange() const
{
	return std::nullopt;
}
