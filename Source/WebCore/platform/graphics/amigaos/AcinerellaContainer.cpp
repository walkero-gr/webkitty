		...
		else
		{
			...
			if (-1 != audioIndex || -1 != videoIndex)
			{
				...
				if (0 != decoderMask)
				{
					...
					MediaPlayerAmigaOSInfo& minfo = m_info;
                    ...
                    if (m_isHLS)
                    {
						...
						for (auto& info : hls->streams())
						{
							MediaPlayerAmigaOSInfoTrack quality;
...
const WebCore::MediaPlayerAmigaOSStreamSettings& Acinerella::streamSettings()
{
	static MediaPlayerAmigaOSStreamSettings defaults;
	if (m_client)
