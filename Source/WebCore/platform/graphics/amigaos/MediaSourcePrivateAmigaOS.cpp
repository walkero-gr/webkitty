...
Ref<MediaSourcePrivateAmigaOS> MediaSourcePrivateAmigaOS::create(MediaPlayerPrivateAmigaOS& parent, MediaSourcePrivateClient& client, const String &url)
{
    auto source = adoptRef(*new MediaSourcePrivateAmigaOS(parent, client, url));
    client.setPrivateAndOpen(source.copyRef());
    return source;
}

MediaSourcePrivateAmigaOS::MediaSourcePrivateAmigaOS(MediaPlayerPrivateAmigaOS& parent, MediaSourcePrivateClient& client, const String &url)
    : m_player(WeakPtr{parent})
    , m_client(client)
    , m_watchdogTimer(RunLoop::current(), this, &MediaSourcePrivateAmigaOS::watchdogTimerFired)
    , m_seekTimer(RunLoop::current(), this, &MediaSourcePrivateAmigaOS::seekInternal)
    , m_seekControlTimer(RunLoop::current(), this, &MediaSourcePrivateAmigaOS::seekControl)
{
	DLIFETIME(dprintf("%s: \n", __PRETTY_FUNCTION__));
	m_url = url.substring(5);
}

MediaSourcePrivateAmigaOS::~MediaSourcePrivateAmigaOS()
{
...
MediaSourcePrivate::AddStatus MediaSourcePrivateAmigaOS::addSourceBuffer(const ContentType& contentType, bool webMParserEnabled, RefPtr<SourceBufferPrivate>& buffer)
{
    ...
    if (MediaPlayerPrivateAmigaOS::extendedSupportsType(parameters, MediaPlayer::SupportsType::MayBeSupported) == MediaPlayer::SupportsType::IsNotSupported)
	{
		return MediaSourcePrivate::AddStatus::NotSupported;
	}

	buffer = MediaSourceBufferPrivateAmigaOS::create(this);
	RefPtr<MediaSourceBufferPrivateAmigaOS> sourceBufferPrivate = static_cast<MediaSourceBufferPrivateAmigaOS*>(buffer.get());
...
void MediaSourcePrivateAmigaOS::onSourceBufferRemoved(RefPtr<MediaSourceBufferPrivateAmigaOS>& buffer)
{
	D(dprintf("%s: \n", __PRETTY_FUNCTION__));
...
void MediaSourcePrivateAmigaOS::durationChanged(const MediaTime& duration)
{
    if (m_player)
		m_player->accSetDuration(duration.toDouble());
}

void MediaSourcePrivateAmigaOS::markEndOfStream(EndOfStreamStatus status)
{
	DEOS(dprintf("%s: \n", __PRETTY_FUNCTION__));
...
void MediaSourcePrivateAmigaOS::unmarkEndOfStream()
{
	DEOS(dprintf("%s: \n", __PRETTY_FUNCTION__));
	m_ended = false;
}

MediaPlayer::ReadyState MediaSourcePrivateAmigaOS::readyState() const
{
//	D(dprintf("%s: \n", __PRETTY_FUNCTION__));
	if (m_player)
...
void MediaSourcePrivateAmigaOS::setReadyState(MediaPlayer::ReadyState rs)
{
...
void MediaSourcePrivateAmigaOS::onSourceBufferLoadingProgressed()
{
...
MediaTime MediaSourcePrivateAmigaOS::duration()
{
	return m_client->duration();
}

MediaTime MediaSourcePrivateAmigaOS::currentMediaTime()
{
...
bool MediaSourcePrivateAmigaOS::isLiveStream() const
{
	return std::isinf(m_client->duration().toFloat());
}

std::unique_ptr<PlatformTimeRanges> MediaSourcePrivateAmigaOS::buffered()
{
	return m_client->buffered();
}

void MediaSourcePrivateAmigaOS::setVolume(double vol)
{
    if (vol != m_volume)
    {
        m_volume = vol;
...
void MediaSourcePrivateAmigaOS::setMuted(bool muted)
{
    if (muted != m_muted)
    {
        m_muted = muted;
...
void MediaSourcePrivateAmigaOS::dumpStatus()
{
...
void MediaSourcePrivateAmigaOS::watchdogTimerFired()
{
	DDUMP(dumpStatus());
...
void MediaSourcePrivateAmigaOS::orphan()
{
	D(dprintf("%s: \n", __PRETTY_FUNCTION__));
	m_orphaned = true;
	m_paintingBuffer = nullptr;
...
void MediaSourcePrivateAmigaOS::warmUp()
{
	DBUFFER(dprintf("%s: \n", __PRETTY_FUNCTION__));
	for (auto& sourceBufferPrivate : m_activeSourceBuffers)
...
void MediaSourcePrivateAmigaOS::coolDown()
{
	DBUFFER(dprintf("%s: \n", __PRETTY_FUNCTION__));
	m_paused = true;
	for (auto& sourceBufferPrivate : m_sourceBuffers)
...
void MediaSourcePrivateAmigaOS::play()
{
	m_paused = false;
...
void MediaSourcePrivateAmigaOS::pause()
{
	m_watchdogTimer.stop();
...
bool MediaSourcePrivateAmigaOS::isSeeking() const
{
	return m_seeking || m_seekCompleted != SeekCompleted;
}

void MediaSourcePrivateAmigaOS::seek(double time)
{
...
void MediaSourcePrivateAmigaOS::seekInternal()
{
...
void MediaSourcePrivateAmigaOS::waitForSeekCompleted()
{
	DSEEK(dprintf("%s: \n", __PRETTY_FUNCTION__));
    if (!m_seeking)
        return;
...
void MediaSourcePrivateAmigaOS::seekCompleted()
{
...
void MediaSourcePrivateAmigaOS::seekControl()
{
...
void MediaSourcePrivateAmigaOS::paint(GraphicsContext& gc, const FloatRect& rect)
{
	if (!!m_paintingBuffer)
		m_paintingBuffer->paint(gc, rect);
}

void MediaSourcePrivateAmigaOS::setOverlayWindowCoords(struct ::Window *w, int scrollx, int scrolly, int mleft, int mtop, int mright, int mbottom, int width, int height)
{
	if (!!m_paintingBuffer)
		m_paintingBuffer->setOverlayWindowCoords(w, scrollx, scrolly, mleft, mtop, mright, mbottom, width, height);
}

const WebCore::MediaPlayerAmigaOSStreamSettings& MediaSourcePrivateAmigaOS::streamSettings()
{
...
void MediaSourcePrivateAmigaOS::onSourceBufferInitialized(RefPtr<MediaSourceBufferPrivateAmigaOS> &)
{
...
void MediaSourcePrivateAmigaOS::onSourceBufferReadyToPaint(RefPtr<MediaSourceBufferPrivateAmigaOS>& buffer)
{
...
void MediaSourcePrivateAmigaOS::onSourceBuffersReadyToPlay()
{
...
void MediaSourcePrivateAmigaOS::onSourceBufferFrameUpdate(RefPtr<MediaSourceBufferPrivateAmigaOS>& buffer)
{
...
void MediaSourcePrivateAmigaOS::onAudioSourceBufferUpdatedPosition(RefPtr<MediaSourceBufferPrivateAmigaOS>&, double position)
{
	if (m_orphaned)
		return;
...
void MediaSourcePrivateAmigaOS::onVideoSourceBufferUpdatedPosition(RefPtr<MediaSourceBufferPrivateAmigaOS>& buffer, double position)
{
	if (m_orphaned)
		return;
...
bool MediaSourcePrivateAmigaOS::areDecodersReadyToPlay()
{
    for (auto& sourceBufferPrivate : m_activeSourceBuffers)
	{
		if (!sourceBufferPrivate->areDecodersReadyToPlay())
...
bool MediaSourcePrivateAmigaOS::areDecodersInitialized()
{
    for (auto& sourceBufferPrivate : m_sourceBuffers)
	{
...
void MediaSourcePrivateAmigaOS::onSourceBufferDidChangeActiveState(RefPtr<MediaSourceBufferPrivateAmigaOS>& buffer, bool active)
{
...
void MediaSourcePrivateAmigaOS::onSourceBufferEnded(RefPtr<MediaSourceBufferPrivateAmigaOS>&)
{
