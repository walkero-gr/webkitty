...
class FloatRect;
class MediaSourcePrivateAmigaOS;
class MediaPlayerPrivateAmigaOS;
class MediaSourceBufferPrivateAmigaOS;

class MediaSourceChunkReader : public WTF::ThreadSafeRefCounted<MediaSourceChunkReader>
{
public:
	typedef Function<void(bool success, WebCore::SourceBufferPrivateClient::InitializationSegment& segment, MediaPlayerAmigaOSInfo& info)> InitializationCallback;
	typedef Function<void(bool)> ChunkDecodedCallback;
protected:
	MediaSourceChunkReader(MediaSourceBufferPrivateAmigaOS *, InitializationCallback &&, ChunkDecodedCallback &&);
public:
	virtual ~MediaSourceChunkReader();

	typedef WTF::StdList<WTF::RefPtr<WebCore::MediaSample>> MediaSamplesList;

    static Ref<MediaSourceChunkReader> create(MediaSourceBufferPrivateAmigaOS *source, InitializationCallback &&icb, ChunkDecodedCallback && ccb) {
...
protected:
	bool initialize();
	void getMeta(WebCore::SourceBufferPrivateClient::InitializationSegment& segment, MediaPlayerAmigaOSInfo& info);
...
class MediaSourceBufferPrivateAmigaOS final : public SourceBufferPrivate, public Acinerella::AcinerellaDecoderClient {
public:
    static Ref<MediaSourceBufferPrivateAmigaOS> create(MediaSourcePrivateAmigaOS*);
    virtual ~MediaSourceBufferPrivateAmigaOS();
	...
	const MediaPlayerAmigaOSInfo &info() { return m_info; }
...
private:
	explicit MediaSourceBufferPrivateAmigaOS(MediaSourcePrivateAmigaOS*);
...
//	bool isActive() const override { return m_isActive; }
	...
	void initialize(bool success, WebCore::SourceBufferPrivateClient::InitializationSegment& segment, MediaPlayerAmigaOSInfo& info);
	void reinitialize(bool success, WebCore::SourceBufferPrivateClient::InitializationSegment& segment, MediaPlayerAmigaOSInfo& info);
	...
	const WebCore::MediaPlayerAmigaOSStreamSettings& streamSettings() override;
...
private:
	MediaSourcePrivateAmigaOS                    *m_mediaSource;
