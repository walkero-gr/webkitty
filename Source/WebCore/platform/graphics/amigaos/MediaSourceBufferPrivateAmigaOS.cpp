...
#include "MediaSourcePrivateAmigaOS.h"
...
MediaSourceChunkReader::MediaSourceChunkReader(MediaSourceBufferPrivateAmigaOS *source, InitializationCallback &&icb, ChunkDecodedCallback &&ccb)
...
void MediaSourceChunkReader::getMeta(WebCore::SourceBufferPrivateClient::InitializationSegment& initializationSegment, MediaPlayerAmigaOSInfo& minfo)
{
	DM(dprintf("[MS]%s\n", __func__));
			...
			case AC_STREAM_TYPE_VIDEO:
				{
					...
					videoTrackInformation.track = VideoTrackPrivateAmigaOSMS::create(m_source, i);
					videoTrackInformation.description = MediaDescriptionAmigaOS::createVideoWithCodec(ac_codec_name(metaCinerella->instance(), i));
			...
			case AC_STREAM_TYPE_AUDIO:
				{
					...
					audioTrackInformation.track = AudioTrackPrivateAmigaOSMS::create(m_source, i);
					audioTrackInformation.description = MediaDescriptionAmigaOS::createAudioWithCodec(ac_codec_name(metaCinerella->instance(), i));
...
void MediaSourceChunkReader::decodeAllMediaSamples()
{
	...
	while (keepDecoding())
	{
		...
		if (package.get() && package->package())
		{
			...
			RefPtr<MediaSampleAmigaOS> mediaSample = MediaSampleAmigaOS::create(package, FloatSize(320, 240), trackID);
...
Ref<MediaSourceBufferPrivateAmigaOS> MediaSourceBufferPrivateAmigaOS::create(MediaSourcePrivateAmigaOS* parent)
{
	D(dprintf("[MS]%s\n", __func__));
	return adoptRef(*new MediaSourceBufferPrivateAmigaOS(parent));
}

MediaSourceBufferPrivateAmigaOS::MediaSourceBufferPrivateAmigaOS(MediaSourcePrivateAmigaOS* parent)
    : m_mediaSource(parent)
{
	...
	m_reader = MediaSourceChunkReader::create(this,
		[this](bool success, WebCore::SourceBufferPrivateClient::InitializationSegment& segment, MediaPlayerAmigaOSInfo& info){
...
MediaSourceBufferPrivateAmigaOS::~MediaSourceBufferPrivateAmigaOS()
{
	DLIFETIME(dprintf("[MS]%s: %p bye!\n", __func__, this));
	clearMediaSource();
}

void MediaSourceBufferPrivateAmigaOS::append(Vector<unsigned char>&&vector)
{
	EP_EVENT(append);
...
void MediaSourceBufferPrivateAmigaOS::appendComplete(bool success)
{
...
void MediaSourceBufferPrivateAmigaOS::willSeek(double time)
{
...
void MediaSourceBufferPrivateAmigaOS::seekToTime(const MediaTime&mt)
{
...
bool MediaSourceBufferPrivateAmigaOS::isSeeking() const
{
    return m_mediaSource && m_mediaSource->isSeeking();
}

void MediaSourceBufferPrivateAmigaOS::signalEOF()
{
	m_reader->signalEOF();
}

void MediaSourceBufferPrivateAmigaOS::setVolume(double vol)
{
	for (int i = 0; i < m_numDecoders; i++)
	{
		if (!!m_decoders[i])
...
void MediaSourceBufferPrivateAmigaOS::clearMediaSource()
{
	DI(dprintf("[MS]%s %p\n", __func__, this));
...
void MediaSourceBufferPrivateAmigaOS::abort()
{
	DI(dprintf("[MS]%s %p\n", __func__, this));

}

void MediaSourceBufferPrivateAmigaOS::terminate()
{
	EP_SCOPE(abort);
...
void MediaSourceBufferPrivateAmigaOS::resetParserState()
{
	D(dprintf("[MS]%s\n", __func__));
	m_appendCompletePending = false;
}

void MediaSourceBufferPrivateAmigaOS::removedFromMediaSource()
{
	D(dprintf("[MS]%s\n", __func__));
	terminate();
	RefPtr<MediaSourceBufferPrivateAmigaOS> me = Ref{*this};
	if (m_mediaSource)
		m_mediaSource->onSourceBufferRemoved(me);
    m_segment = WebCore::SourceBufferPrivateClient::InitializationSegment();
}

void MediaSourceBufferPrivateAmigaOS::onTrackEnabled(int index, bool enabled)
{
...
void MediaSourceBufferPrivateAmigaOS::dumpStatus()
{
	int numDec = 0;
	for (int i = 0; i < m_numDecoders; i++)
	{
...
void MediaSourceBufferPrivateAmigaOS::getFrameCounts(unsigned& decoded, unsigned &dropped) const
{
...
void MediaSourceBufferPrivateAmigaOS::flush(const AtomString& trackID)
{
...
void MediaSourceBufferPrivateAmigaOS::becomeReadyForMoreSamples(int index)
{
...
void MediaSourceBufferPrivateAmigaOS::flush()
{
	D(dprintf("[MS]%s\n", __func__));
...
void MediaSourceBufferPrivateAmigaOS::enqueueSample(Ref<MediaSample>&&sample, const AtomString&)
{
	auto msample = static_cast<MediaSampleAmigaOS *>(&sample.get());
...
void MediaSourceBufferPrivateAmigaOS::allSamplesInTrackEnqueued(const AtomString&)
{
	D(dprintf("[MS]%s\n", __func__));
	m_eos = true;
...
bool MediaSourceBufferPrivateAmigaOS::isReadyForMoreSamples(const AtomString&)
{
// 	D(dprintf("[MS]%s %d\n", __func__, m_readyForMoreSamples));
	return m_readyForMoreSamples;
}

void MediaSourceBufferPrivateAmigaOS::setActive(bool isActive)
{
	D(dprintf("[MS]%s\n", __func__));
    if (m_mediaSource)
    {
...
void MediaSourceBufferPrivateAmigaOS::notifyClientWhenReadyForMoreSamples(const AtomString&trackID)
{
}

bool MediaSourceBufferPrivateAmigaOS::canSetMinimumUpcomingPresentationTime(const AtomString&) const
{
	DBR(dprintf("[MS]%s\n", __func__));
	// WARNING: HACK
	// SourceBuffer calls this after a batch of enqueueSample calls
	auto *me = const_cast<MediaSourceBufferPrivateAmigaOS *>(this);
...
MediaPlayer::ReadyState MediaSourceBufferPrivateAmigaOS::readyState() const
{
	D(dprintf("[MS]%s\n", __func__));
	if (m_mediaSource)
...
void MediaSourceBufferPrivateAmigaOS::setReadyState(MediaPlayer::ReadyState rs)
{
	D(dprintf("[MS]%s %d\n", __func__, int(rs)));
	if (m_mediaSource)
...
void MediaSourceBufferPrivateAmigaOS::initialize(bool success,
	WebCore::SourceBufferPrivateClient::InitializationSegment& segment,
	MediaPlayerAmigaOSInfo& minfo)
{
...
void MediaSourceBufferPrivateAmigaOS::reinitialize(bool success,
	WebCore::SourceBufferPrivateClient::InitializationSegment& segment,
	MediaPlayerAmigaOSInfo& minfo)
{
	RefPtr<Acinerella::AcinerellaPointer> acinerella = m_reader->acinerella();
...
void MediaSourceBufferPrivateAmigaOS::warmUp()
{
	D(dprintf("[MS] warmUp\n"));
...
void MediaSourceBufferPrivateAmigaOS::coolDown()
{
	dispatch([this]() {
		for (int i = 0; i < m_numDecoders; i++)
		{
...
void MediaSourceBufferPrivateAmigaOS::threadEntryPoint()
{
	while (auto function = m_queue.waitForMessage())
	{
		(*function)();
...
void MediaSourceBufferPrivateAmigaOS::dispatch(Function<void ()>&& function)
{
	ASSERT(isMainThread());
	ASSERT(!m_queue.killed() && m_thread);
	m_queue.append(makeUnique<Function<void ()>>(WTFMove(function)));
...
void MediaSourceBufferPrivateAmigaOS::performTerminate()
{
	D(dprintf("[MS]%s: %p\n", __func__, this));
...
void MediaSourceBufferPrivateAmigaOS::play()
{
	D(dprintf("%s: \n", __PRETTY_FUNCTION__));
	EP_EVENT(play);
	dispatch([this] {
...
void MediaSourceBufferPrivateAmigaOS::prePlay()
{
	D(dprintf("%s: \n", __PRETTY_FUNCTION__));
	EP_EVENT(play);
	dispatch([this] {
...
void MediaSourceBufferPrivateAmigaOS::pause()
{
	D(dprintf("%s: \n", __PRETTY_FUNCTION__));
	EP_EVENT(pause);
	dispatch([this] {
...
const WebCore::MediaPlayerAmigaOSStreamSettings& MediaSourceBufferPrivateAmigaOS::streamSettings()
{
	static WebCore::MediaPlayerAmigaOSStreamSettings defaults;
	if (m_mediaSource)
	{
...
void MediaSourceBufferPrivateAmigaOS::onDecoderWarmedUp(RefPtr<Acinerella::AcinerellaDecoder>)
{
	D(dprintf("%s: allreadytoplay %d\n", __PRETTY_FUNCTION__, areDecodersReadyToPlay()));
}

void MediaSourceBufferPrivateAmigaOS::onDecoderReadyToPlay(RefPtr<Acinerella::AcinerellaDecoder>)
{
...
void MediaSourceBufferPrivateAmigaOS::onDecoderPlaying(RefPtr<Acinerella::AcinerellaDecoder>, bool)
{
	D(dprintf("%s: \n", __PRETTY_FUNCTION__));

}

void MediaSourceBufferPrivateAmigaOS::onDecoderUpdatedBufferLength(RefPtr<Acinerella::AcinerellaDecoder>, double)
{
	D(dprintf("%s: \n", __PRETTY_FUNCTION__));

}

void MediaSourceBufferPrivateAmigaOS::onDecoderUpdatedPosition(RefPtr<Acinerella::AcinerellaDecoder> decoder, double position)
{
...
void MediaSourceBufferPrivateAmigaOS::onDecoderUpdatedDuration(RefPtr<Acinerella::AcinerellaDecoder>, double duration)
{
	// live streams
}

MediaTime MediaSourceBufferPrivateAmigaOS::duration() const
{
	return MediaTime::createWithDouble(m_info.m_duration);
}

MediaTime MediaSourceBufferPrivateAmigaOS::currentMediaTime() const
{
	if (m_mediaSource)
...
void MediaSourceBufferPrivateAmigaOS::onDecoderEnded(RefPtr<Acinerella::AcinerellaDecoder> decoder)
{
...
void MediaSourceBufferPrivateAmigaOS::paint(GraphicsContext& gc, const FloatRect& rect)
{
...
void MediaSourceBufferPrivateAmigaOS::setOverlayWindowCoords(struct ::Window *w, int scrollx, int scrolly, int mleft, int mtop, int mright, int mbottom, int width, int height)
{
	DI(dprintf("[MS]%s: %p decoder %p\n", __func__, this, m_paintingDecoder.get()));
	if (!!m_paintingDecoder)
	{
...
void MediaSourceBufferPrivateAmigaOS::setAudioPresentationTime(double apts)
{
	if (!!m_paintingDecoder)
	{
		Acinerella::AcinerellaVideoDecoder *decoder = static_cast<Acinerella::AcinerellaVideoDecoder *>(m_paintingDecoder.get());
...
bool MediaSourceBufferPrivateAmigaOS::areDecodersReadyToPlay()
{
	for (int i = 0; i < m_numDecoders; i++)
	{
		if (!!m_decoders[i])
...
bool MediaSourceBufferPrivateAmigaOS::areDecodersPlaying()
{
	for (int i = 0; i < m_numDecoders; i++)
	{
...
float MediaSourceBufferPrivateAmigaOS::decodersBufferedTime()
{
	float buffer = -1.f;
...
void MediaSourceBufferPrivateAmigaOS::onDecoderWantsToRender(RefPtr<Acinerella::AcinerellaDecoder> decoder)
{
...
void MediaSourceBufferPrivateAmigaOS::onDecoderNotReadyToRender(RefPtr<Acinerella::AcinerellaDecoder> decoder)
{
	if (decoder == m_paintingDecoder)
		m_paintingDecoder = nullptr;
}

void MediaSourceBufferPrivateAmigaOS::onDecoderRenderUpdate(RefPtr<Acinerella::AcinerellaDecoder> decoder)
{
	if (decoder == m_paintingDecoder)
	{
		WTF::callOnMainThread([this, protect = Ref{*this}, decoder]() {
