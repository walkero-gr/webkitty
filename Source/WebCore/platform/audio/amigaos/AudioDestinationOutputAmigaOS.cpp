...
#if ENABLE(WEB_AUDIO)
#include "AudioDestinationOutputAmigaOS.h"
#include <proto/ahi.h>
...
AudioDestinationOutputAmigaOS::AudioDestinationOutputAmigaOS(AudioDestinationRenderer &parent)
	: m_renderer(parent)
{
	for (int i = 0; i < 2; i++)
	{
...
AudioDestinationOutputAmigaOS::~AudioDestinationOutputAmigaOS()
{
	ahiCleanup();
}

bool AudioDestinationOutputAmigaOS::start()
{
	if (!ahiInit())
		return false;
...
void AudioDestinationOutputAmigaOS::stop()
{
	D(dprintf("[AD]%s:\n", __func__));
	if (m_ahiControl)
...
bool AudioDestinationOutputAmigaOS::ahiInit()
{
	D(dprintf("[AD]%s:\n", __func__));
		...
		if ((m_ahiIO = reinterpret_cast<AHIRequest *>(CreateIORequest(m_ahiPort, sizeof(AHIRequest)))))
		{
			...
			if (0 == OpenDevice(AHINAME, AHI_NO_UNIT, reinterpret_cast<IORequest *>(m_ahiIO), 0))
			{
				...
				static struct EmulLibEntry GATE_SoundFunc = {
					TRAP_LIB, 0, (void (*)(void))AudioDestinationOutputAmigaOS::soundFunc
				};
...
void AudioDestinationOutputAmigaOS::ahiCleanup()
{
	D(dprintf("[AD]%s:\n", __func__));
...
void AudioDestinationOutputAmigaOS::soundFunc()
{
	AHIAudioCtrl *ahiCtrl = reinterpret_cast<AHIAudioCtrl *>(REG_A2);
	AudioDestinationOutputAmigaOS *me = reinterpret_cast<AudioDestinationOutputAmigaOS *>(ahiCtrl->ahiac_UserData);
...
void AudioDestinationOutputAmigaOS::ahiThreadEntryPoint()
{
