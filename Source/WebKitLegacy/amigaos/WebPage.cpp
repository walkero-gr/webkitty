...
#warning "This needs to be enabled"
// #if OS(AMIGAOS)
// #define __USE_INLINE__
// #endif
...
public:
	...
	void repaint(RastPort *rp, const int outX, const int outY)
	{
		...
		m_damage.visitDamagedTiles([&](const int x, const int y, const int width, const int height) {
			...
			WritePixelArray(src, x, y, stride, PIXF_A8R8G8B8, rp, outX + x, outY + y, width, height);
		});
	}
	...
	void repaintAll(RastPort *rp, const int outX, const int outY)
	{
		...
		WritePixelArray(src, 0, 0, stride, PIXF_A8R8G8B8, rp, outX, outY, m_width, m_height);
	}
...
//			cairo_set_source_rgb(cairo, 0.0f, 1.0, 1.0);
	...
	WritePixelArray(src, 0, 0, stride, PIXF_A8R8G8B8, rp, paintX, paintY,
...
void WebPage::drawDragImage(struct RastPort *rp, const int x, const int y, const int width, const int height)
{
	...
	if (!!imageRef)
	{
		...
		WritePixelArray(src, 0, 0, stride, PIXF_A8R8G8B8, rp, x, y, width, height);
	}
}
